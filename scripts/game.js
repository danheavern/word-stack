        ...
        if (!row.collided && rowCollision(row)){
            if (charCollision(row)) {
                ...
                if (LETTERS.includes(row.text)) {
                    ...
                    const needed = needLetter(row.text)
                    removeLetter(row)
                    insertBlock(block)
...
function reset() {
    ...
    document.getElementById('game-over').classList.replace('d-flex', 'd-none')
...
function start() {
    running = true
    document.getElementById('start-screen').classList.remove('d-flex')
...
function setGameOver() {
    gameOver = true
    document.getElementById('game-over').classList.replace('d-none', 'd-flex')
